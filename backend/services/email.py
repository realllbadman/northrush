...
def _shell(kicker, headline, hero_label, hero_value, sections_html, preheader,
    ...
    phone_line = f' &middot; Call or text {_esc(BUSINESS_PHONE)}' if BUSINESS_PHONE else ""
...
def format_customer_inquiry(inq) -> str:
    ...
    lines += [f"Questions? Call or text us on {BUSINESS_PHONE}" if BUSINESS_PHONE else "",
...
def format_customer_booking(bk) -> str:
    ...
    return "\n".join(l for l in [
        ...
        f"Need us sooner? Call or text us on {BUSINESS_PHONE}" if BUSINESS_PHONE else "",
        f"{BUSINESS_NAME} · {BUSINESS_HOURS}",
    ] if l)
